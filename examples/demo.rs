...
fn main() {
    ...
    // Encode an X-Address 
    let x_address = xpring.encode_classic_address("rU6K7V3Po4snVhBBaU29sesqs2qTQJWDw1", Some(12345), None)?;
    println!("X-Address {}", x_address); 

    // Decode an X-Address
    let classic_address = xpring.decode_x_address("XVfC9CTCJh6GN2x8bnrw3LtdbqiVCUvtU3HnooQDgBnUpQT")?;
    println!("\nClassic Address {:?}", classic_address); 

    // Address Validation
    let is_wrong_address_valid = xpring.validate_address("rU6K7V3Po4snVhBBaU29sesqs2qTQJWDw1rU6K7V3Po4snVhBBaU29sesqs2qTQJWDw1")?;
    println!("\nis this Address rU6K7V3Po4snVhBBaU29sesqs2qTQJWDw1rU6K7V3Po4snVhBBaU29sesqs2qTQJWDw1 valid? {}", is_wrong_address_valid);
    
    // X Address Validation
    let is_x_address_valid = xpring.validate_address("TVr7v7JGN5suv7Zgdu9aL4PtCkwayZNYWvjSG23uMMWMvzZ")?;
    println!("\nis this Address TVr7v7JGN5suv7Zgdu9aL4PtCkwayZNYWvjSG23uMMWMvzZ valid? {}", is_x_address_valid);

    // Classic Address Validation
    let is_classic_address_valid = xpring.validate_address("rU6K7V3Po4snVhBBaU29sesqs2qTQJWDw1")?;
    println!("\nis this Address rU6K7V3Po4snVhBBaU29sesqs2qTQJWDw1 valid? {}", is_classic_address_valid);
    ...
    // Sign a message
    let signed_message = xpring.wallet_sign("mymessage".to_owned(), "000974B4CFE004A2E6C4364CBF3510A36A352796728D0861F6B555ED7E54A70389".to_owned())?;
    ...
    println!("\nSigned Message Verification {:?}", message_verification_result);

    // Account Balance
    let balance = xpring.get_balance("TVr7v7JGN5suv7Zgdu9aL4PtCkwayZNYWvjSG23uMMWMvzZ")?;

    println!("\nBalance for TVr7v7JGN5suv7Zgdu9aL4PtCkwayZNYWvjSG23uMMWMvzZ is {}", balance);

    //Send Payment
    println!("\nSending payment...");
    let sending_wallet = xpring.wallet_from_seed("shKtxFAYfNUHYayYMYkp3KjQQX2UY".to_owned(), None, true)?;
